...
* Use the `quick_cluster` function to check if any potential mutations are randomly distributed between clusters
...
getasso <- function(m1,m2) {
      if (m1 == m2) return(list(p.value = NA, estimate=NA))
      testm <- matrix( data = c(
        sum(binary[,m1] == 1 & binary[,m2] == 1),
        sum(binary[,m1] == 1 & binary[,m2] == -1),
        sum(binary[,m1] == -1 & binary[,m2] == 1),
        sum(binary[,m1] == -1 & binary[,m2] == -1)
      ), ncol = 2  )
...
alleleRatio <- M/(M+N)
binary <- apply(alleleRatio, 2, function(x) ifelse(is.na(x),0, ifelse(x>0.05,1,-1)))

totest <- colnames(binary)[1:11]
#totest <- unique(calls[,"mainClone"])
tests <- lapply(include, function(marker) lapply(totest, getasso , m2= marker))
```
